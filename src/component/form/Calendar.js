    ...
    render() {
        return (
            <div className="calendarSection">
                <h2 className="titleDate">Choissisez vos jours de livraison</h2>
                <div className="divCalendar"onClick={this.reset}>
                ...
                <button className= "buttonDeliveryDate" onClick={this.validation}>Confirmer</button>
                {/* Est-ce que shoxDate est true ou false? si c'est true les deux paragraphe s'affiche, si c'est false on ne montre : rien*/}
                {this.state.showDate ? (
                    <div>
                        <p className="paraDateDelivery m-0"> Vous serez livré.e selon nos possibilités à partir du : {this.state.date[0].toLocaleDateString()} jusqu'au : {this.state.date[1].toLocaleDateString()}</p>
                    </div>
                ): null}
